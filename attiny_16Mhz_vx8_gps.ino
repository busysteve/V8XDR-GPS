...
/* Led output pin deifinitions.
   All the leds are connectd to PORTB as follows:
...
void loop(void)
{
  ...
  //SerialGPS.println("$GPRMC,095142.196,V,4857.5953,N,00811.9616,E,9999.99,999.99,080810,,*2C");
  ////Serial.println("$GPRMC,095142.196,V,4857.5953,N,00811.9616,E,9999.99,999.99,080810,,*2C");
  //SerialGPS.println("$GPGGA,095142.196,4957.5953,N,00811.9616,E,0,00,99.9,00234.7,M,0047.9,M,000.0,0000*42");
  ////Serial.println("$GPGGA,095142.196,4957.5953,N,00811.9616,E,0,00,99.9,00234.7,M,0047.9,M,000.0,0000*42");
  //return;
  ...
  { 
    ...
    if(rx_byte = SerialGPS.read())
    {
      tbp_byte = rx_byte;
      rx_byte = NO_DATA;
    }

    //Serial.write( tbp_byte );
  }
  */


/*
$GPRMC,092656.00,A,3404.94146,N,08401.34441,W,0.039,,051121,,,D*63
$GPVTG,,T,,M,0.039,N,0.073,K,D*28
$GPGGA,092656.00,3404.94146,N,08401.34441,W,2,10,0.86,331.7,M,-31.5,M,,0000*66
$GPGSA,A,3,12,02,20,17,19,46,09,05,06,25,,,1.75,0.86,1.53*0A
$GPGSV,4,1,16,02,61,309,33,04,04,048,,05,23,204,23,06,50,035,29*75
$GPGSV,4,2,16,09,15,081,17,11,67,317,34,12,50,296,39,17,22,106,27*7C
$GPGSV,4,3,16,19,41,093,25,20,56,198,34,24,02,242,07,25,21,316,32*79
$GPGSV,4,4,16,28,16,161,,46,28,241,33,48,31,237,35,51,43,218,31*7E
$GPGLL,3404.94146,N,08401.34441,W,092656.00,A,D*76
  ...
  { 
    ...
    case START_TX:
      ...
      if (!tx_has_data)
      {
        ...
        //for (uint8_t i = 0; (tx_buffer[i] != 0) && (i < BUFFER_SIZE); i++) 
        {
          //SerialGPS.write( tx_buffer[i] );
          SerialGPS.write( tx_buffer );
        }
        //SerialGPS.write( '\n' );
...
bool process_field(void)
{
  ...
  case NONE: /* Determine command type */
    if (sentence_cycle == 1 && rx_buffer[3] == 'G' && rx_buffer[4] == 'G' && rx_buffer[5] == 'A')
    {
      rx_command = GGA;
      sentence_cycle = 2;
      break;
    }
    if (sentence_cycle == 2 && rx_buffer[3] == 'R' && rx_buffer[4] == 'M' && rx_buffer[5] == 'C')
    {
      rx_command = RMC;
      sentence_cycle = 1;
      break;
    ...
    /*
    if (sentence_cycle == 3 && rx_buffer[3] == 'Z' && rx_buffer[4] == 'D' && rx_buffer[5] == 'A')
    {
      rx_command = ZDA;
      sentence_cycle = 1;
      break;
  ...
  case GGA:
    ...
    case 0x01:
      ...
      if (rx_field_size == 0)
      {
        //PORTB |= GGA_RED; /* Turn the red LED on. */
        res = false;
        break;
      }
    ...
    case 0x02:
      if (rx_field_size == 0)
        ;//PORTB |= GGA_RED; /* Red LED on. */
      else
        ;//PORTB |= GGA_GREEN; /* Green LED on. */
  ...
  case RMC:
    ...
    case 0x01:
      ...
      if (rx_field_size == 0)
      {
        //PORTB |= RMC_RED; /* Turn the red LED on. */
        res = false;
        break;
      }
      /* Time field is fixed to 10 characters: hhmmss.sss */
    ...
    case 0x03:
      if (rx_field_size == 0)
        ; //PORTB |= RMC_RED; /* Red LED on. */
      else
        ; //PORTB |= RMC_GREEN; /* Green LED on. */
      /* Latitude field is fixed to 9 characters: ddmm.ssss */
      fix_decimal_field_len(&rx_buffer[rx_buf_pos - rx_field_size], 4, 4);
      rx_buf_pos -= rx_field_size;
      rx_buf_pos += 9;
